export function renderAsciiTable(headers: string[], rows: string[][]): string {
    ...
    const colWidths = headers.map((h, i) => {
        ...
        return max + 2; // +2 for left/right padding space
    });
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        // Ensure space at the start and pad the rest
        const rowStr = `║` + rowData.map((d, colIdx) => ' ' + padRight(d || '', colWidths[colIdx] - 1)).join(`║`) + `║`;
        result += rowStr + '\n';
        if (i < rows.length - 1) {
            result += midSep + '\n';
        }
